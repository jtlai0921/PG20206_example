/* 2003/7/20
 *
 * sniffer.c - [Ethernet packet capture] for Windows/Linux
 * 
 * [how to compile]
 *   > cl sniffer_win32.c ws2_32.lib  (Windows)
 *   # gcc -o sniffer sniffer.c       (Linux)
 *
...
{
	printf("Detected CTRL+C signal. terminate program\n");
...
void caputure_for_windows(void)
{
	WSADATA wsaData;
	...
	/* capture packet */
	for (;;) {
		size = recv(sock, (char *)recvbuf, sizeof(recvbuf), 0);
		if (size > 0) {
